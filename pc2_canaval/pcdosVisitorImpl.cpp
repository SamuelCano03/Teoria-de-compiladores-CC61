...
std::any pcdosVisitorImpl::visitPrintExpr(pcdosParser::PrintExprContext *ctx) {
    ...
    if (exprResult.has_value() && exprResult.type() == typeid(llvm::AllocaInst*)) {
        // The value is castable to llvm::Value*
        llvm::AllocaInst *allocaInst = std::any_cast<llvm::AllocaInst*>(exprResult);
        // Now you can safely use allocaInst
        builder->CreateRet(allocaInst);
    } else {
        // The value is not castable to llvm::Value*
        std::cerr << "Error: Unable to cast to llvm::AllocaInst*" << std::endl;
